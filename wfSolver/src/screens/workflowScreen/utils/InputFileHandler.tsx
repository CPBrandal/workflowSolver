...
function validateAndFixOutgoingConnections(nodes: WorkflowNode[]): void {
  ...
  for (let level = 0; level < maxLevel; level++) {
    ...
    for (const node of nodesInLevel) {
      ...
      if (availableTargets.length > 0) {
        ...
        console.warn(
          `Could not find target for node ${node.name} at level ${level} in uploaded workflow`
        );
      }
    }
  }
  ...
  for (const node of finalLevelNodes) {
    ...
    if (node.connections.length !== originalConnectionCount) {
      console.log(
        `Cleaned up ${originalConnectionCount - node.connections.length} invalid final-level connections for ${node.name}`
      );
    }
  }
