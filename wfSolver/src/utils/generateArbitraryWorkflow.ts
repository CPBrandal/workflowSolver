  ...
  export function generateArbitraryWorkflow(config: ArbitraryWorkflowConfig): WorkflowNode[] {
    ...
    console.log('Generating arbitrary workflow:', { 
      nodeCount, maxWidth, maxDepth, edgeProbability, maxEdgeSpan, gammaParams
    });
    ...
    try {
      ...
      console.log('Generated workflow with levels:', nodes.map(n => ({ id: n.id, level: n.level, name: n.name, duration: n.executionTime })));
      ...
      for (let targetLevel = sourceLevel + 1; targetLevel <= maxTargetLevel; targetLevel++) {
        ...
        for (const targetNode of targetNodes) {
          if (Math.random() < adjustedProbability) {
            ...
            const edgeExists = sourceNode.connections.some(edge => 
